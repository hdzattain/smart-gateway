...
# no source patching). Upstream "New API" attribution is preserved per
...
set_opt() {
  curl -s -b "$COOKIES" -H "New-Api-User: $ADMIN_ID" -H 'Content-Type: application/json' \
...
set_opt '{"key":"Footer","value":"© 2026 Smart Gateway. Powered by <a href=\"https://github.com/QuantumNous/new-api\">New API</a> (open-source, Apache-2.0)."}'
