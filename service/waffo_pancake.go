...
// are exposed separately for self-service flows driven from smart-gateway's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("smart-gateway-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "smart-gateway-store"
	defaultWaffoPancakeProductName = "smart-gateway-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because smart-gateway has no renewal-
// event handling; Pancake auto-renewing without smart-gateway extending user
