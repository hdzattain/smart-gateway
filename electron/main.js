...
function saveAndOpenErrorLog() {
  try {
    ...
    const logFileName = `smart-gateway-crash-${timestamp}.log`;
    ...
    // 写入日志
    const logContent = `Smart Gateway 崩溃日志
...
function analyzeError(errorLogs) {
  ...
  if (allLogs.includes('failed to start HTTP server') || 
    ...
    return {
      ...
      solution: `可能的解决方案：\n\n1. 关闭占用端口 ${PORT} 的其他程序\n2. 检查是否已经运行了另一个 Smart Gateway 实例\n3. 使用以下命令查找占用端口的进程：\n   Mac/Linux: lsof -i :${PORT}\n   Windows: netstat -ano | findstr :${PORT}\n4. 重启电脑以释放端口`
    };
  }
  ...
  if (allLogs.includes('database is locked') || 
    ...
    return {
      ...
      solution: '可能的解决方案：\n\n1. 检查是否已经打开了另一个 Smart Gateway 窗口\n   - 查看任务栏/Dock 中是否有其他 Smart Gateway 图标\n   - 查看系统托盘（Windows）或菜单栏（Mac）中是否有 Smart Gateway 图标\n\n2. 如果刚刚关闭过应用，请等待 10 秒后再试\n\n3. 重启电脑以释放被占用的文件\n\n4. 如果问题持续，可以尝试：\n   - 退出所有 Smart Gateway 实例\n   - 删除数据目录中的临时文件（.db-shm 和 .db-wal）\n   - 重新启动应用'
    };
  }
...
function getBinaryPath() {
  ...
  if (isDev) {
    const binaryName = platform === 'win32' ? 'smart-gateway.exe' : 'smart-gateway';
  ...
  switch (platform) {
    case 'win32':
      binaryName = 'smart-gateway.exe';
      break;
    case 'darwin':
      binaryName = 'smart-gateway';
      break;
    case 'linux':
      binaryName = 'smart-gateway';
      break;
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    env.SQLITE_PATH = path.join(dataDir, 'smart-gateway.db');
